...
在做研究输出（问卷、访谈、画像、竞品分析、报告）时，prompt 中**显式带上知识库引用关键词**，LLM 会先检索 kb-wiki 已有内容、向你展示找到的相关页面、再综合产出，并标注每段引用来源。
...
| 关键词 | 示例 |
|--------|------|
| 「参考知识库」「结合知识库」「用知识库」 | "参考知识库，帮我设计一份问卷" |
| 「参考 wiki」「结合 wiki」「用 wiki 里的」 | "结合 wiki 里的回流玩家资料，写访谈提纲" |
| 「基于 kb-wiki」「拉一下知识库」 | "基于 kb-wiki 已有研究，做个 persona" |
| 「结合已有资料」「基于现有研究」 | "结合已有访谈资料，写本周研究周报" |

#### 5 个具体调用示例

```
"参考知识库，帮我设计一份针对回流玩家的支付满意度问卷"
"结合 wiki 里关于回流玩家的资料，写一份访谈提纲"
"基于 kb-wiki 已有的用户研究，做一个高活跃付费玩家的 persona"
"拉一下知识库的竞品资料，写一份和竞品 X 的支付流程对比报告"
"用我们已有的访谈数据，写本周研究周报，重点关注加载速度问题"
```

#### LLM 的执行流程

```
1️⃣ 检索 → 调用 qmd hybrid 搜索 entities/concepts/synthesis
2️⃣ 展示已知 → 列出找到的相关页面，等你确认或补充
3️⃣ 综合产出 → 用知识库内容 + 通用知识做创作
              引用知识库的部分标注 [来源: wiki/concepts/痛点-加载速度.md]
              通用补充部分标注 (通用经验补充，知识库无对应资料)
4️⃣ 可选归档 → 询问是否归档到 wiki/synthesis/，让创作也成为知识库积累
```

> 💡 **不带关键词不会主动检索**：直接说"帮我设计问卷"时 LLM 走通用知识，不打扰；只有显式引用知识库时才进入此工作流——给你完全的掌控权。

> 📚 完整规则参考 [SKILL.md「作为研究输出的知识源」章节](SKILL.md#作为研究输出的知识源显式触发)
