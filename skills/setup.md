...
### 步骤 12：预下载 AI 搜索模型（用户选择）

**询问用户是否现在下载**：

```
📥 是否现在预下载 AI 搜索模型？（共约 2GB，只需下载一次）
...
⚠️ 如果选择【跳过】：
   ✅ 可用：/ingest、/lint、/query（BM25 关键词搜索）
   ❌ 不可用：语义搜索（理解同义词/近义表达）、LLM 重排序（结果精排）
   📉 影响：搜索只能精确匹配关键词，无法理解语义。
        例如搜 "玩家流失" 找不到只写了 "用户留存下降" 的页面。

💡 可以跳过，后续执行 /query 时会再次提示下载。
```
...
- `Vectors: N embedded`（N > 0）→ ✅ 成功
- `Vectors: 0 embedded` → ⚠️ 未完成，稍后重试
...
```markdown
## [{{CURRENT_DATE}}] setup | AI 模型下载已跳过（仅 BM25 可用）
```

直接进入步骤 13（欢迎信息），但在欢迎信息中追加提醒：

```
⚠️ AI 搜索模型未下载，当前仅支持 BM25 关键词搜索。
   首次执行 /query 时会再次询问是否下载。
```
