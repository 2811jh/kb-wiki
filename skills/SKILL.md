...
└── wiki/              ← Wiki 层（LLM 完全掌控，自动生成和维护）
    ...
    ├── synthesis/     ← 综合分析页面（洞察归档、跨资料结论）
    ├── index.md       ← 内容目录（每次 ingest 自动更新）
    └── log.md         ← 操作日志（append-only，不可删改）
```

**三层职责**：
- **raw/**：原始输入，不可变，是知识的来源
- **wiki/**：知识的提炼产物，LLM 负责构建和维护所有交叉引用、矛盾标注、综合结论
- **CLAUDE.md**：规范层，告诉 LLM 如何工作，随使用逐步演进
...
| 用户命令 | 执行内容 | 参考文档 |
|---------|---------|---------|
| `/setup` | 首次初始化：检测环境、创建目录、生成 CLAUDE.md、配置 qmd | [setup.md](setup.md) |
| `/ingest <文件路径或内容>` | 导入资料，自动更新 10-15 个 wiki 页面 | [ingest.md](ingest.md) |
| `/query <问题>` | 搜索知识库，生成综合答案，可选归档到 synthesis/ | [query.md](query.md) |
| `/lint` | 健康检查：矛盾、孤立页面、过时论断、缺失引用、数据空白 | [lint.md](lint.md) |
| `/status` | 显示 wiki 统计 + qmd 索引状态（运行 `qmd status`） | — |
...
1. **LLM 负责写，用户负责读**：用户永远不需要手动编写任何 Wiki 内容
