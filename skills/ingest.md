...
如果文件 frontmatter 包含 `clipped: true` 或 `source_url` 字段，识别为 Web Clipper 剪藏文章：

- **来源记录**：在 sources/ 摘要页中自动记录 `source_url` 原始链接
- **来源类型**：frontmatter 中标注 `tags` 包含 `网络文章`
- **图片检查**：扫描文件中的图片引用，如果存在远程 URL（`http://` 或 `https://`），提醒用户：
  ```
  ⚠️ 发现 3 张远程图片未本地化，建议在 Obsidian 中按 Ctrl+Shift+D 下载到本地。
  远程图片可能在未来失效。是否继续导入？
  ```
...
### 步骤 2.5：读取 Schema.md 命名规范（批量导入时尤为关键）

> ⚠️ **强制规则**：创建任何 wiki 页面之前，必须先读取知识库根目录下的 `Schema.md`，确认命名规范和页面模板。
> 这是防止命名不一致的关键步骤，**即使是单次导入也不可跳过**。
...
> 💡 **批量导入时**：如果调度子任务/并行处理，必须将 Schema.md 的命名规范**完整传递**给每个子任务，不能只传递"创建页面"的笼统指令。

---

### 步骤 3：在 wiki/sources/ 写摘要页面

在 `{{WIKI_PATH}}/wiki/sources/` 创建新页面，文件名采用 **`{类型前缀}-{描述}.md`** 格式（如 `访谈-汕头服务器内测.md`）。
...
你可以对我说"对知识库进行健康检查"，我会帮你检测矛盾论断、孤立页面、过时内容、缺失交叉引用等问题。
定期健康检查能保持知识库的质量和一致性。
---
```
...
当用户积攒了多篇 Web Clipper 剪藏文章后，支持批量导入：

### 触发方式

- 用户主动说"把新文章都导入"、"处理 raw/ 里的新文件"
- 或 LLM 在会话启动时自动检测到未处理文件并提醒

### 检测未处理文件

```bash
# 1. 从 log.md 提取所有已 ingest 的来源文件
grep "来源文件" "{{WIKI_PATH}}/wiki/log.md" | sed 's/.*来源文件：//'

# 2. 列出 raw/ 下所有文件
find "{{WIKI_PATH}}/raw" -type f -not -name ".*" -not -name "*.DS_Store"

# 3. 对比找出未处理文件
```

**简化判断**：也可以直接扫描 `raw/articles/` 目录，将文件列表展示给用户确认哪些需要导入。

### 批量处理流程

1. 列出所有待导入文件，展示给用户确认
2. 用户确认后，**逐个执行完整的 ingest 10 步流程**（不可跳步）
3. 每完成一个文件，简要报告进度：
   ```
   ✅ [1/3] 访谈-游戏社区运营策略.md — 新建 1 个 sources + 更新 3 个 entities + 2 个 concepts
   ✅ [2/3] 报告-用户留存分析框架.md — 新建 1 个 sources + 新建 1 个 concepts + 更新 1 个 synthesis
   ⏳ [3/3] 处理中...
   ```
4. 全部完成后输出汇总 + 统一更新 index.md + 运行 `qmd update` + `qmd embed`

> ⚠️ 批量导入时，每个文件的 ingest 都必须先读取 Schema.md 命名规范（步骤 2.5），确保命名一致性。

---

## 文件命名规范

Wiki 页面文件名采用 **`{类型前缀}-{描述}.md`** 纯中文格式。
...
3. 不匹配 → 自主创建新前缀（2-3 个字，概括该类内容的本质）
4. 新前缀自动成为该知识库的惯例，后续同类页面沿用
```

> 💡 前缀是**活约定**而非死目录。优先复用已有前缀，没有合适的才新建。

#### 初始前缀参考（仅供起步）

| 目录 | 常见前缀 | 示例 |
|------|---------|------|
| concepts/ | `痛点-`、`行为-`、`需求-` … | `痛点-加载速度.md` |
| sources/ | `问卷-`、`访谈-`、`竞品-`、`报告-` … | `问卷-春节满意度.md` |
| entities/ | `用户-`、`产品-`、`组织-` … | `用户-女性玩家.md` |
| synthesis/ | `对比-`、`洞察-`、`概览-` … | `对比-基岩与Java版.md` |

> `…` 表示 LLM 可自由扩展。新前缀使用 3 次以上后，建议在 Schema.md 中正式记录。

**规则**：
1. **优先复用**已有前缀，保持一致性
2. **自主新建**：没有合适前缀时自行创建（2-3 个字）
3. 描述简洁（2-8 个字），纯中文
4. 避免特殊字符（`/ \ : * ? " < > |`）
